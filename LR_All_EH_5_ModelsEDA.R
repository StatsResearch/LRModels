...
log.filename<-paste(sep='',INSTALL_DIR,'/PhDStuff/ThesisSoftware/Logs/LR-All-EH5-output-',smallname,'-EH5.log')
log.level<-3

# Start things off 
LogWrite('LR_All_EH5_ModelsEDA.R Starting ...',3)
...
TTG_DIR<-paste(sep='',INSTALL_DIR,'/PhDStuff/ThesisSoftware/TrgTestGenerator/output_All_EH5')
...
running.auc.model.all.EH5<-vector()
...
plotFileName<-paste(sep='',INSTALL_DIR,'/PhDStuff/ThesisSoftware/LRModels/output_All_EH5/ROC_All_EH5_withAUC.pdf')
...
print('Processing: All EH_5')
for(sequence in seq(1,num.repeats))
{
    trg.filename<-paste(sep='',TTG_DIR,'/All_Training_EH_5_seq_',sequence, '.csv')
    ...
    attach(all.trg)
        ...
        model.all.EH5<-glm(case_label~ Age + Sex 
        ...
        signals<-unlist(attr(model.all.EH5$R,'dimnames')[1])
        if(first.run == TRUE)
        {
        ...
        model.res$model.all.EH5<-list()
        model.res$model.all.EH5$name<-'model.all.EH5'
        model.res$model.all.EH5$sequence<-sequence
    
        model.res$model.all.EH5$aic<-summary(model.all.EH5)$aic
        model.res$model.all.EH5$null.deviance<-summary(model.all.EH5)$null.deviance
        model.res$model.all.EH5$deviance<-summary(model.all.EH5)$deviance
    
        model.res$model.all.EH5$coef<-summary(model.all.EH5)$coefficients[,1]
        model.res$model.all.EH5$se<-summary(model.all.EH5)$coefficients[,2]
    
        signif<-summary(model.all.EH5)$coefficients[,4]
        model.res$model.all.EH5$signif<-signif
        model.res$model.all.EH5$selection<-lapply(signif,SelectionThreshold,selection.cutoff)
        running.selection.col.names[sequence+1]<-paste(sep='','run_',sequence)
        running.selection<-cbind(running.selection,model.res$model.all.EH5$selection)
...
#         model.res$model.mean.sd.quadratic$selection<-lapply(summary(model.mean.sd.quadratic)$coefficients[,4],SelectionThreshold,selection.cutoff)
    ...
    test.filename<-paste(sep='',TTG_DIR,'/All_Test_EH_5_seq_',sequence,'.csv')
    ...
    # ------------ Prediction phase ---------- #
    pred.results<-AssessModel(model.all.EH5,model.input.test,all.test$case_label)
    
    model.res$model.all.EH5$auc<-pred.results$auc
    running.auc.model.all.EH5[sequence]<-pred.results$auc
...
avg.auc.model.all.EH5<-mean(running.auc.model.all.EH5)
sd.auc.model.all.EH5<-sd(running.auc.model.all.EH5)
...
res.info$auc.array.model.all.EH5<-running.auc.model.all.EH5
res.info$avg.auc.model.all.EH5<-avg.auc.model.all.EH5
res.info$sd.auc.model.all.EH5<-sd.auc.model.all.EH5
colnames(running.selection)<-running.selection.col.names
running.selection<-cbind(running.selection,selection.totals)
res.info$selection.matrix.model.all.EH5<-running.selection
#res.info$selection.totals.model.all.EH5<-selection.totals
res.info$selection.totals.model.all.EH5<-cbind(signals,selection.totals)


cat('avg.auc.model.all.EH5:',avg.auc.model.all.EH5,'\n')
cat('sd.auc.model.all.EH5:',sd.auc.model.all.EH5,'\n')
#-----
...
JSON.filename<-paste(sep='',INSTALL_DIR,
                     '/PhDStuff/ThesisSoftware/LRModels/','output_All_EH5/ROC-',smallname,'-All-EH-5-Info.json')
