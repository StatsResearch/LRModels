...
# SVN: $Id: Build_LRModels.R 662 2012-06-24 22:00:51Z rob $
...
{
    event.horizon<-10
    model.file<-'MinModel-V1-20120412.R'
    mode<-'3X2'
    num.repeats<-10
}
...
for(window.size in seq(5,30,by=5))
{
    ...
    for(sequence in seq(1,num.repeats))
    {
        ...
        signals<-unlist(attr(running.model$R,'dimnames')[1])
        if(first.run == TRUE)
        {   
        ...
        model.res$model$coef<-summary(running.model)$coefficients[,1]
        ...
        model.res$model$selection<-lapply(signif,SelectionThreshold,selection.cutoff)
        ...
        # ------------ Prediction phase ---------- #
        pred.results<-AssessModel(running.model,model.input.test,all.test$case_label)
